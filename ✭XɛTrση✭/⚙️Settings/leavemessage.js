...
Set the leave message <per server>!`;
...
\`\`\`**`),
    ...
    if (text !== "off") {
      ...
      if (data) {
        ...
        newData.save();
        message.reply(`Leave Message set to \n${newData.ByeMsg}`);
      } else if (!data) {
        let newData = new prefixModel({
          ByeMsg: args.join(" "),
          Ӽɛȶʀօռֆɨɖ: message.guild.id,
      ...
      if (data2) {
        ...
        return message.reply(`Leave Message has been turned off!`);
      } else if (!data2) {
        return message.reply(`Leave Message isn't setup!`);
      }
    }
  },
};
